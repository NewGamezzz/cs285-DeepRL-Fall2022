...
class MLPPolicy(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def forward(self, observation: torch.FloatTensor) -> Any:
        if self.discrete:
            ...
            # used parameterization trick
            mean_ac = self.mean_net(observation)
            std_ac = torch.exp(self.logstd)
            rand_term = torch.randn(mean_ac.shape)

            sample_ac = mean_ac + std_ac * rand_term
            return sample_ac
