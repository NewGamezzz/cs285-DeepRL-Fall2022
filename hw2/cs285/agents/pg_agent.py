...
class PGAgent(BaseAgent):
    ...
    def calculate_q_vals(self, rewards_list):
        ...
        if not self.reward_to_go:
            for rewards in rewards_list:
                discounted_rewards = self._discounted_return(rewards)
                q_values.append(discounted_rewards)

        # Case 2: reward-to-go PG
        # Estimate Q^{pi}(s_t, a_t) by the discounted sum of rewards starting from t
        else:
            for rewards in rewards_list:
                discounted_rewards = self._discounted_cumsum(rewards)
                q_values.append(discounted_rewards)
    ...
    def estimate_advantage(self, obs: np.ndarray, rews_list: np.ndarray, q_values: np.ndarray, terminals: np.ndarray):
        ...
        if self.nn_baseline:
            ...
            values_norm = (values_unnormalized - values_unnormalized.mean()) / values_unnormalized.std()
            values = values_norm * q_values.std() + q_values.mean()
    ...
    def _discounted_return(self, rewards):
        ...
        discounted_gamma = np.power(self.gamma, np.arange(T))
        discounted_reward = np.dot(discounted_gamma, rewards)

        list_of_discounted_returns = np.array([discounted_reward] * T)
    ...
    def _discounted_cumsum(self, rewards):
        ...
        for t in range(T):
            discounted_gamma = np.power(self.gamma, np.arange(T-t))
            discounted_reward = np.dot(discounted_gamma, rewards[t:])

            list_of_discounted_cumsums.append(discounted_reward)
